...
describe('_initWorker', () => {
  ...
  test('rejects gracefully when setParameters always fails (null API)', async () => {
    ...
    // Attach rejection handler immediately to prevent unhandled-rejection noise
    let caughtErr;
    promise.catch((e) => { caughtErr = e; });

    // Advance time past all retry delays (outer exponential + inner retries)
    await jest.runAllTimersAsync();

    expect(caughtErr).toBeDefined();

    // State must be clean so a future retry is possible
    expect(ocr.worker).toBeNull();
    expect(ocr.workerReady).toBe(false);

    // The user-facing status should mention an OCR engine error
  ...
  test('succeeds on outer retry when first worker is broken', async () => {
    const bad = brokenWorker();
    const good = healthyWorker();
    let callIndex = 0;
    Tesseract.createWorker.mockImplementation(() => {
      callIndex++;
      return Promise.resolve(callIndex === 1 ? bad : good);
    });
    ...
    expect(ocr.worker).toBe(good);
    expect(ocr.workerReady).toBe(true);
    // The broken worker must have been terminated
...
describe('_startOCR (triggered by Start OCR click)', () => {
  ...
  test('does not throw when _initWorker fails', async () => {
    Tesseract.createWorker.mockImplementation(() =>
      Promise.resolve(brokenWorker()),
    );
